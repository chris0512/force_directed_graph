d3.csv('./senate_committee_data.csv', function(d, i, headers) {
  ...
  let linkGp = svg.append('g')
      .classed("links", true);

  let nodeGp = svg.append('g')
      .classed("nodes", true);
  ...
  function setUpCheckboxes(committees) {
    let boxAreas = d3.select("#checkboxes")
        .selectAll("div")
        .data(committees)
        .enter()
        .append("div");
    ...
    boxAreas
        ...
        .on("click", () => {
          let activeCommittees = committees.filter(c => d3.select(`input[value="${c}"]`)
              .property("checked"));
